...
const Categories = ({ setCategoryId, categoryId, showLabel }) => {
    ...
    const deleteCategoryData = async (catId) => {
        const response = await axios.delete(`http://13.233.1.96:9092/product/category/deleteCategory/${catId}`, {
            headers: {
                'Content-Type': 'application/json'
            }
        })
    ...
    return (
        ...
        <Box bg={colors.backgroundGray} w="auto" p={6} m="auto">
            ...
            <Box bg={colors.white} borderRadius="lg" height="auto" marginLeft={showLabel ? "280px" : "100px"} padding='20px' fontFamily='Poppins, sans-serif'>
                                    ...
                                    <>
                                        <Tr key={index}>
                                            <Td><Image height="70px" width="70px"
                                                src={`http://13.233.1.96:9092/product/category/categoryImage/${currElem.catId}`} alt={`not found`}
                                            ...
                                            <Td><Icon as={FaEdit} fill="blue" ml="10px" onClick={() => edit(currElem.catId)} cursor="pointer" height="50px" /></Td>
                                            <Td><Icon as={FaTrashAlt} onClick={() => deleteCategory(currElem.catId)} fill="red" ml="10px" cursor="pointer" height="50px" /></Td>
                                        </Tr>
                                    </>
                                )
                            })}
                ...
                {showCategoryDialogueBox && <Modal isOpen={isOpen} onClose={onClose}>
                    <ModalOverlay />
                    <ModalContent>
